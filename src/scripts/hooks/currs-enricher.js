export const CurrsEnricher = {
    listen: () => {
        Hooks.on('setup', () => {
            CONFIG.TextEditor.enrichers.push({
                pattern: /\@Currs\[((\d+)|([0-9a-zA-Z\-=. ]*?))( noname)?\]/gim,
                enricher: async (match, options) => {
                    const [_, input, __, ___, noname] = match;
                    const data = {
                        cls: ["content-link", "currs"],
                    };
                    ...
                    if(!item) {
                        ...
                        data.title = `${amount} currs`;
                        data.dataset = {amount};
                    }
                    else {
                        data.inner = `Purchase ${noname === undefined ? `${item.name} ` : ""}(<span>${item.system.cost}</span>)`;
                        data.title = `Purchase ${item.name} (${item.system.cost} currs)`;
        ...
        const activateListeners = (htmlElement) => {
            const anchors = htmlElement.querySelectorAll("a.currs");
            for (const anchor of anchors) {
                const {amount, item} = anchor.dataset;
                anchor.addEventListener("dragstart", ev => {
                    ev.stopPropagation();
                    ev.dataTransfer.setData("text/plain", JSON.stringify({type: "currs", data: {amount, item}}));
                });
                if(item) {
                    ...
                    anchor.addEventListener("click", async ev => {
                        ...
                        ui.notifications.info(`${actor.name} Gained ${amount} currs (New Total: ${actor.system.money})`);
                    });
                }
            }
        }
